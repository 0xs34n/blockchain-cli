...
class Blockchain {
  ...
  replaceChain (newBlocks) {
    if (!this.isValidChain(newBlocks) || newBlocks.length <= this.blockchain.length) {
      cli.log('Received blockchain invalid')
      return
    }

    cli.log('Received blockchain is valid. Replacing current blockchain with received blockchain')
  ...
  isValidNewBlock (newBlock, previousBlock) {
    ...
    if (previousBlock.index + 1 !== newBlock.index) {
      cli.log('invalid index')
      return false
    } else if (previousBlock.hash !== newBlock.previousHash) {
      cli.log('invalid previous hash')
      return false
    } else if (blockHash !== newBlock.hash) {
      cli.log(typeof (newBlock.hash) + ' ' + typeof blockHash)
      cli.log(`invalid hash: ${blockHash} ${newBlock.hash}`)
      return false
    }
    return true
  }
